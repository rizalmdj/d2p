...
class Admin extends CI_Controller {
	...
	public function pengguna() {
		...
		$project_name			= addslashes($this->input->post('project_name'));
		$project_id				= addslashes($this->input->post('project_id'));
		$project_manager		= addslashes($this->input->post('project_manager'));
		...
		if ($mau_ke == "act_edt") {
			if ($this->upload->do_upload('file_d2p')) {
				$up_data	 	= $this->upload->data();
		...
		} else if($mau_ke == 'act_insert') {
			// if ($this->upload->do_upload('file_d2p')) {
			// 	$up_data	 	= $this->upload->data();
				
				$this->db->query("INSERT INTO tr_request VALUES (NULL,'$name', '$project_name', '$project_id','$project_manager', '$keterangan', '$date', NOW(),'1',NOW() , '1', '1')");
